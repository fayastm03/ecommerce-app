...
class PosterProvider extends ChangeNotifier {
    ...
    try {
      if (selectedImage == null) {
        SnackBarHelper.showErrorSnackBar(
            'Please select an image for the poster.');
        return;
      }

      Map<String, dynamic> formDataMap = {
        'posterName': posterNameCtrl.text,
        'img': 'no_data', //image added from server side
      };
  ...
  updatePoster() async {
    try {
      Map<String, dynamic> formDataMap = {
        'posterN ame': posterNameCtrl.text,
  ...
  deletePoster(Poster poster) async {
    try {
      ...
      if (response.isOk) {
        ApiResponse apiResponse = ApiResponse.fromJson(response.body, null);
        if (apiResponse.success == true) {
          SnackBarHelper.showSuccessSnackBar('Poster Deleted Succesfully');
          _dataProvider.getAllPosters();
        }
      } else {
        SnackBarHelper.showErrorSnackBar(

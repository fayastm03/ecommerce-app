...
class CartProvider extends ChangeNotifier {
  ...
  checkCoupon() async {
    try {
      ...
      List<String> productIds =
          myCartItems.map((cartItems) => cartItems.productId).toList();
  ...
  addOrder(BuildContext context) async {
    try {
      Map<String, dynamic> order = {
        "userID": _userProvider.getLoginUser()?.sId ?? '',
        "orderStatus": 'pending',
  ...
  submitOrder(BuildContext context) async {
    if (selectedPaymentOption == 'cod') {
      ...
      await razorpayPayment(operation: () => addOrder(context));
    }
  }
  ...
  clearCartItems() {
    flutterCart.cartItemsList.clear();
    myCartItems = [];
    notifyListeners();
  }
